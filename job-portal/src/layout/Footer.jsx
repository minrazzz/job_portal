import React from "react";

export const Footer = () => {
   return (
      <>
         <div>
            <footer className="Footer mt-40 bg-[#057E01]  shadow dark:bg-[#1E2936] bg-[#047E01] h-[50%]">
               <div className="w-full max-w-screen-xl mx-auto p-4 md:py-8">
                  <div className="sm:flex sm:items-center sm:justify-between">
                     <a href="/home" className="flex items-center mb-4 sm:mb-0">
                        <img
                           src="/logo.png"
                           className="h-[80px] mr-3"
                           alt="Job-Seeker"
                        />
                        <span className="self-center text-2xl font-semibold font-fair whitespace-nowrap text-white">
                           JOB-SEEKER
                        </span>
                     </a>
                     <ul className="flex flex-wrap items-center mb-6 text-sm font-medium text-gray-500 sm:mb-0 dark:text-gray-400">
                        <li>
                           <a
                              href="#"
                              className="mr-4 hover:underline md:mr-6 text-white"
                           >
                              About
                           </a>
                        </li>
                        <li>
                           <a
                              href="#"
                              className="mr-4 hover:underline md:mr-6 text-white"
                           >
                              Privacy Policy
                           </a>
                        </li>
                        <li>
                           <a
                              href="#"
                              className="mr-4 hover:underline md:mr-6 text-white"
                           >
                              Licensing
                           </a>
                        </li>
                        <li>
                           <a href="#" className="hover:underline text-white">
                              Contact
                           </a>
                        </li>
                     </ul>
                  </div>
                  <hr className="my-6 border-gray-200 sm:mx-auto dark:border-gray-700 lg:my-8" />
                  <span className="block text-sm text-gray-500 sm:text-center dark:text-gray-400 text-white">
                     © 2023{" "}
                     <a href="/" className="hover:underline text-white">
                        Job-Seeker™
                     </a>
                     . All Rights Reserved.
                  </span>
               </div>
            </footer>
         </div>
      </>
   );
};
